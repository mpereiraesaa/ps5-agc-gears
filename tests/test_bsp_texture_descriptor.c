...
int main(void)
{
    ...
    assert(bsp_gfx1013_combined_descriptor(
               ...
               1u, BSP_TEXTURE_REPEAT, BSP_TEXTURE_FILTER_BILINEAR) == 0);
    ...
    assert(bsp_gfx1013_combined_descriptor(
               ...
               1u, BSP_TEXTURE_CLAMP_LAST_TEXEL,
    ...
    assert(bsp_gfx1013_combined_descriptor(
               ...
               1u, BSP_TEXTURE_REPEAT, BSP_TEXTURE_FILTER_POINT) == -1);
    assert(bsp_gfx1013_combined_descriptor(
               descriptor, UINT64_C(0x0000123456789a00), 65u, 8u, 256u,
               1u, BSP_TEXTURE_REPEAT, BSP_TEXTURE_FILTER_POINT) == -1);

    const BspBundleTexture textures[2] = {
        {.offset = 0u, .bytes = 4608u, .width = 64u, .height = 8u,
         .row_pitch = 256u, .format = BSP_BUNDLE_IMAGE_RGBA8_UNORM,
         .name_hash = 1u, .flags = 0u, .mip_count = 7u},
        {.offset = 4608u, .bytes = 4096u, .width = 17u, .height = 8u,
         .row_pitch = 256u, .format = BSP_BUNDLE_IMAGE_RGBA8_UNORM,
         .name_hash = 2u, .flags = 0u, .mip_count = 5u},
    };
    ...
    assert(bsp_texture_build_tables(
               ...
               UINT64_C(0x0000123500000000),
               BSP_TEXTURE_FILTER_TRILINEAR, &written) == -2);
    assert(written == 0u);
    assert(bsp_texture_build_tables(
               tables, required, &bundle,
               UINT64_C(0x0000123400000000),
               UINT64_C(0x0000123500000000),
               BSP_TEXTURE_FILTER_TRILINEAR, &written) == 0);
    assert(written == required);
    assert(tables[0] == UINT32_C(0x34000000));
    assert(tables[BSP_GFX1013_COMBINED_DWORDS + 8u] == UINT32_C(0x92));
    assert(tables[BSP_TEXTURE_TABLE_DWORDS] == UINT32_C(0x34000012));
    ...
    assert(bsp_texture_build_tables(
               ...
               UINT64_C(0x0000123400000000),
               UINT64_C(0x0000123500000000),
               BSP_TEXTURE_FILTER_TRILINEAR, &written) == -4);
    assert(written == 0u);
    return 0;
}
