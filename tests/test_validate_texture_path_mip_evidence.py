...
VALIDATOR = ROOT / "tools" / "validate_texture_path_mip_evidence.py"
BUNDLE_SHA = "3" * 64
...
def make_evidence(directory: Path) -> Path:
    ...
    messages = [
        ...
        f"BSP_TEXTURE_PATH_BOOT schema=1 slice=mip-sampler target=gfx1013 fw=test transient_slots=2 ownership=fence+videoout bundle_sha256={BUNDLE_SHA} bundle_bytes=42 soak_frames=10000",
        ...
        "MIP_CHAINS_READY textures=164 layout=addr-sw-linear order=smallest-to-base pitch_alignment=256 levels_min=5 levels_max=9 chain_bytes=6000000 deterministic=box-rne",
        "BSP_TEXTURE_TABLE_LAYOUT textures=164 descriptor_dwords=3936 storage=per-frame-transient lightmap=512x760 base_mode=repeat lightmap_mode=clamp base_filter=alternating-trilinear-anisotropic4x lightmap_filter=bilinear composition=base_x_lightmap",
        "BSP_LOOP_BEGIN mode=texture-path-mip-soak buffers=2 color_dma=false depth_dma=true indexed=true frames=10000 mip_layout=addr-sw-linear samplers=trilinear+anisotropic4x lightmap_pattern=paired descriptors=per-frame overlay=fixed retirement=fence+videoout",
        ...
        "MIP_SAMPLER_FRAME frame=0 slot=0 filter=trilinear lightmap_pattern=0 s0=00000000 s1=00600000 s2=08500000 s3=00000000",
        ...
        "MIP_SAMPLER_FRAME frame=1 slot=1 filter=anisotropic4x lightmap_pattern=0 s0=00410400 s1=08600000 s2=28f00000 s3=00000000",
        f"DYNAMIC_LIGHTMAP_FRAME frame=9998 slot=0 pattern=1 patch_hash=2222222222222222 {dynamic} total_uploaded_bytes=900000000",
        "MIP_SAMPLER_FRAME frame=9998 slot=0 filter=trilinear lightmap_pattern=1 s0=00000000 s1=00600000 s2=08500000 s3=00000000",
        f"DYNAMIC_LIGHTMAP_FRAME frame=9999 slot=1 pattern=1 patch_hash=2222222222222222 {dynamic} total_uploaded_bytes=900096256",
        "MIP_SAMPLER_FRAME frame=9999 slot=1 filter=anisotropic4x lightmap_pattern=1 s0=00410400 s1=08600000 s2=28f00000 s3=00000000",
        ...
        "MIP_SAMPLER_READBACK trilinear_buffer=3333333333333333 anisotropic4x_buffer=5555555555555555 bytes=64 lightmap_pattern=1 paired=true framebuffer_distinct=true guards=intact frames=10000",
        ...
        "BSP_TEXTURE_PATH_MIP_COMPLETE frames=10000 textures=164 layout=addr-sw-linear t_sharp=mip-aware samplers=trilinear+anisotropic4x readback=gpu-visible paired_lightmap=true tokens=exact guards=intact errors=0",
...
def main() -> int:
    with tempfile.TemporaryDirectory() as temporary:
        ...
        assert summary["frames"] == 10_000 and summary["textures"] == 164
        ...
        path = make_evidence(directory)
        with (directory / "synthetic.log").open("ab") as handle:
            handle.write(b"tamper")
        integrity = run_validator(path)
        assert integrity.returncode != 0 and "size/hash" in integrity.stderr
    print("texture-path mip evidence validator tests passed")
    return 0
