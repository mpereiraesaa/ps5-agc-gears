...
VALIDATOR = ROOT / "tools" / "validate_bsp_noclip_evidence.py"
BUNDLE_SHA = "1" * 64
...
def make_evidence(directory: Path) -> Path:
    messages = [
        ...
        f"BSP_NOCLIP_BOOT schema=1 target=gfx1013 fw=test bundle_sha256={BUNDLE_SHA} bundle_bytes=42 soak_frames=10000",
        "BSP_NOCLIP_PAD_READY sticks=dual triggers=vertical connected_required=true",
        "BSP_LOOP_BEGIN mode=noclip-soak buffers=2 color_dma=false depth_dma=true indexed=true frames=10000",
        "BSP_VIDEOOUT_TOKEN frame=0 buffer=0 expected=100 observed=100 exact=true",
    ]
    for frame in range(599, 10_000, 600):
        connected = frame + 1
        messages.append(
            "BSP_NOCLIP_INPUT "
    ...
    messages += [
        "BSP_VIDEOOUT_TOKEN frame=9999 buffer=1 expected=101 observed=101 exact=true",
        "BSP_FRAME frame=9999 completed=10000 compose_avg_ns=1 gpu_wait_avg_ns=1 "
        ...
        "BSP_NOCLIP_READBACK buffer0=3333333333333333 buffer1=3333333333333333 "
        "bytes=64 bright_pixels0=1 bright_pixels1=1 guards=intact frames=10000 errors=0",
        "BSP_NOCLIP_SOAK_COMPLETE frames=10000 connected_frames=10000 moving_frames=10000 "
        "looking_frames=10000 distance_milli=10000000 input_changes=10000 "
        "input_hash=4444444444444444 read_errors=0 tokens=exact guards=intact errors=0",
    ]
    ...
    transcript = "\n".join(
        ...
        + records
        + [f"BYE seq={len(records)} reason=bsp-noclip-soak-complete", ""]
    ).encode()
    ...
    manifest = {
        ...
        "bye_fields": {"seq": str(len(records)), "reason": "bsp-noclip-soak-complete"},
        "log_path": log_name,
...
def main() -> int:
    with tempfile.TemporaryDirectory() as temporary:
        ...
        assert json.loads(valid.stdout)["frames"] == 10_000

        data = json.loads(manifest.read_text())
        log_path = directory / data["log_path"]
        original = log_path.read_bytes()
        log_path.write_bytes(original.replace(b"connected_frames=10000", b"connected_frames=9999"))
        ...
        assert "connected_frames" in semantic_failure.stderr
    ...
    print("bsp noclip evidence validator tests passed")
    return 0
