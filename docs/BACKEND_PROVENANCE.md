...
| `ps5_gfx1013_descriptor` | Builds named V#, T# and S# records for structured vertices, raw constants and BSP images/samplers. | Exact DWORD tests pin the independently authored subset derived from Mesa's public GFX10.3 definitions. |
| `ps5_cache_contract` | Plans an aligned CPU flush and GCR invalidation before GPU reads, then requires fence plus exact VideoOut token before CPU reuse. | Host tests pin the 256-byte range, `0x00004380` GCR value, operation order and refusal of fence-only completion. |
| `bsp_resource_frame` and `bsp_resource_draw` | Build per-frame constants, descriptors and overlay geometry, then compose the map and overlay pipeline permutations. | Synthetic GPU mappings verify all spans, exact draw counts, packet lengths and no-write-on-invalid-input behavior. |
| `shaders/bsp_resource.pipe`, `shaders/bsp_overlay.pipe` and generated permutation metadata | Define the constant-buffer textured map and transient visible overlay pipelines. | Source contracts pin resource mappings and vertex layouts; LLPC manifests and generated application-word counts are validated together. |
